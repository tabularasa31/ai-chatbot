...
| `apiBase` | `https://getchat9.live` | Override only on staging/dev |
| `widgetBase` | `<script origin>/v1/` | Origin of the widget UI bundle. Defaults to the loader script's own origin. Override only if you serve a custom widget build. |
...
In the snippet below, `apiBase` and `color` come from your own application's config — substitute literal strings (e.g. `apiBase: "https://api.example.com"`, `color: "#a855f7"`) or values from your existing branding/config object. They are not provided by Chat9.

```js
// At app startup, after you've fetched any branding/config:
Chat9Widget.start({
  apiBase: "https://api.example.com",  // your dashboard origin (or omit to use default)
  color: "#a855f7"                     // your brand color (or omit for the default gradient)
});
