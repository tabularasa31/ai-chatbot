...
- **Needs attention:** the default view — visitors waiting for a human (an open escalation ticket nobody has answered yet, longest wait first) and conversations an operator is currently holding. Once you have replied and handed the conversation back to the bot it leaves this view, even if the ticket is still open. The count of visitors waiting shows as a badge next to **Inbox** in the sidebar.
