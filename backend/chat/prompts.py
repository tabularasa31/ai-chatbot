...
def build_rag_prompt(
    ...
    *,
    ...
    system_rules = (
        ...
        "- When (and ONLY when) your reply ends on a question you expect the user to answer, append the literal marker `<clarifying/>` as the very last token, after all natural-language text. It is machine-readable and stripped before the reply is shown; the backend reads it to know a question was asked, so a reply that asks something without it is treated as a plain answer. Never emit `<clarifying/>` and `<needs_human/>` on the same reply — a turn either asks the user something or hands off, never both.\n"
