...
For identified visitors (those passing `user_id` or `email` in `userHints`), the conversation also resumes across devices and after `localStorage` is cleared: the server reattaches the visitor to their most recent open conversation. Anonymous visitors (no `userHints`) always start a fresh session, and a conversation that has already ended is never resumed — a new session begins instead.
