...
def _get_answer_for_message(
    ...
    up/down is only written on assistant turns in the chat API. The answer text
    is redacted: it ends up in FAQ candidates generated by an OpenAI call.
    ...
    **with no other user message in between**. Taking simply the next
    ...
    would be fed to OpenAI as FAQ candidates carrying that single answer's
    thumbs-up/down. A visitor turn nobody answered has no answer, and
    returning ``None`` drops it from the candidate set, which is correct.
    """
