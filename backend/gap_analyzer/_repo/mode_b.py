"""Mode B persistence: clusters, question embeddings, per-tenant retrieval."""
...
from backend.gap_analyzer._repo.mode_a import mode_a_embedding_rows
...
def list_unclustered_mode_b_questions(
    db: Session, tenant_id: UUID
) -> list[ModeBQuestionRecord]:
    rows = (
...
def list_mode_b_clusters(db: Session, tenant_id: UUID) -> list[ModeBClusterRecord]:
    rows = (
        db.query(GapCluster)
...
def update_mode_b_question_embedding(
    ...
    *,
    ...
    question = db.get(GapQuestion, question_id)
    if question is None:
        logger.warning(
...
def update_mode_b_cluster(
    ...
    *,
    ...
    capabilities = _repository_capabilities(db)
    cluster = db.get(GapCluster, cluster_id)
    if cluster is None:
