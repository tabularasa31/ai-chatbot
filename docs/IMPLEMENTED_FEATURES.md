...
| Knowledge API (Profile + FAQ) | Tenant knowledge endpoints for profile and FAQ moderation: `GET/PATCH /knowledge/profile`, `GET /knowledge/faq`, `POST /knowledge/faq/{id}/approve`, `POST /knowledge/faq/{id}/reject`, `POST /knowledge/faq/approve-all`, `PUT/DELETE /knowledge/faq/{id}`; extraction status exposed as profile field; public profile terminology uses `topics` for extracted doc themes | `backend/knowledge/routes.py`, `backend/knowledge/schemas.py`, `backend/main.py`, `backend/models.py`, `tests/test_knowledge_api.py` |
