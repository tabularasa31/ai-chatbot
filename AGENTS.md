...
- **Liveness alerts.** Two independent signals, both raising Sentry issues through the usual alert rules: a Sentry uptime monitor probes `https://api.getchat9.live/health` every minute from outside (three failures = down; it must target the custom domain — Sentry refuses the shared `*.railway.app` domain), and the API sends a Sentry Crons check-in every minute (`backend/observability/heartbeat.py`, monitor `api-heartbeat`, started in the lifespan). Error capture alone is not a liveness signal: it lives inside the process that is not running when a deploy never reaches `uvicorn`.
