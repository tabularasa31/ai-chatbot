...
description: How Chat9 chooses the language to reply in, and how to control it via browser locale, userHints, and dashboard settings.
---
...
- **The default greeting (shown before the user types anything)** comes out in the language picked by this chain, in order: the `locale` you passed via `userHints` → the browser's reported locale → English.
...
1. **`userHints.locale`** — if you passed `userHints` to `Chat9Widget.start({...})` (or to `Chat9Widget.setHints({...})`) and included a `locale`, that wins. It's the strongest signal because your own system told us about this specific visitor.
...
The greeting itself is rendered in the language picked from this chain. For example, if you pass `userHints.locale: "es-ES"`, the greeting opens in Spanish; if you pass nothing and the user's browser is in `de-DE`, the greeting opens in German; if neither is available, the greeting opens in English.
...
The lock lasts for the whole conversation, and a conversation has no end the user can trigger: there is no "start new chat" control, and the conversation only rolls over into a fresh one after a week without messages (or when your integration starts a new session). The new conversation goes through the full chain again and may settle on a different language. So an end user cannot switch the bot's language mid-conversation. This is intentional: real bilingual switches mid-conversation are rare in support, and the lock prevents the much-more-common annoyance of the bot flip-flopping between languages on noisy turns.
...
### From the widget integration — `userHints.locale`

If your tenant frontend already knows the visitor's preferred language (for example, a registered locale on their account profile), pass it via `userHints.locale` to `Chat9Widget.start({...})` (or `Chat9Widget.setHints({...})` after the widget is mounted). Chat9 will use it as the bootstrap greeting language and as a stronger initial signal than the browser default. See [Embedding the Widget → userHints](/docs/embedding-the-widget#userhints).
...
- **Mixed-language input** ("Hello, ¿necesito ayuda?") — Chat9 picks the dominant signal and replies on that side. Strong English markers like greetings can win even when the rest of the message is in another language. If the chat ends up locked to a language the user didn't expect, the lock holds until the conversation rolls over after a week of silence — or until your integration starts a new session.
...
- Use **Dashboard → Inbox** to see the language each turn was answered in. The conversation transcript shows the actual reply text, which is the easiest signal to verify quickly.
- If you're passing `userHints.locale`, sanity-check the value — Chat9 normalizes locale tags such as `es-MX`, `pt-BR`, `de-AT` automatically.
