...
- Embedding pipeline (`backend/embeddings/service.py`):
  - `chunk_text(text, chunk_size=500, overlap_sentences=1)` — sentence-aware chunks + `char_offset`/`char_end` in metadata (обновлено 2026-03-21, FI-009)
...
> **Актуализация 2026-03-21:** базовый FI-009 (sentence-aware chunking + `metadata` с позицией и файлом) реализован в `backend/embeddings/service.py`. Пункты плана выше про FI-009 — исторический снимок; следующая волна — структурный/токеновый chunking и Small-to-Big (см. `docs/BACKLOG_RAG_QUALITY.md`).
