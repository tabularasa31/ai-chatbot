...
For visitors identified by a `user_id` in `userHints`, the conversation also resumes across devices and after `localStorage` is cleared: the server reattaches the visitor to their most recent open conversation. This applies to `user_id` only — an `email`-only hint always starts a fresh session, since an email is too easily guessed to safely reattach to someone's conversation. Anonymous visitors (no `userHints`) always start a fresh session, and a conversation that has already ended is never resumed — a new session begins instead.

Escalating to a human agent does not end the conversation. If the visitor's email is already known, the bot goes on answering their questions normally while the ticket is open. If it is not — the default for anonymous visitors — the bot asks for an email address and keeps asking until it gets one, because support replies by email. Either way the chat is marked as ended only when the visitor confirms they need nothing further: after the handoff the bot asks whether it can help with anything else, and the visitor declines. At that point the transcript stays visible; a "Start new chat" button appears for the visitor to open a new session.
