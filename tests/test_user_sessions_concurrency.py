...
def _active_sessions(db: Session, *, client_id: uuid.UUID, user_id: str) -> list[UserSession]:
    return (
        ...
        .order_by(UserSession.session_started_at.asc(), UserSession.id.asc())
        .all()
    )
...
def test_start_user_session_two_writer_race_returns_winner_row(engine, db_session: Session, monkeypatch) -> None:
    ...
    def _raise_duplicate(*args, **kwargs):
        raise user_session_service.IntegrityError("stmt", {}, Exception("duplicate active row"))

    monkeypatch.setattr(user_session_service, "_close_active_user_sessions", _no_close)
    monkeypatch.setattr(user_session_service, "_create_user_session_row", _raise_duplicate)
...
def test_start_user_session_close_then_race_creates_one_new_active_session(
    ...
    original_close = user_session_service._close_active_user_sessions
    ...
    def _inject_conflict(*args, **kwargs):
        ...
        monkeypatch.setattr(user_session_service, "_create_user_session_row", _inject_conflict)
        monkeypatch.setattr(user_session_service, "_close_active_user_sessions", _no_close)
        raise user_session_service.IntegrityError("stmt", {}, Exception("duplicate active row"))
    ...
    refreshed_old = db_session.get(UserSession, old_row.id)
