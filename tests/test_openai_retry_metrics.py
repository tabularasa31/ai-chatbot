...
@pytest.mark.parametrize(
    "call_type_kwargs,expected_call_type",
    [({}, "chat_completion"), ({"call_type": "embedding"}, "embedding")],
    ids=["default-chat_completion", "explicit-embedding"],
)
def test_attempt_event_includes_call_type(captured_events, call_type_kwargs, expected_call_type):
    """openai_retry.attempt includes call_type, defaulting to chat_completion."""
    call_openai_with_retry("chat_generate", lambda: "ok", tenant_id="tnt_test", **call_type_kwargs)

    attempts = _named(captured_events, "openai_retry.attempt")
    assert len(attempts) == 1
    assert attempts[0]["properties"]["call_type"] == expected_call_type
...
@pytest.mark.parametrize(
    "emit_chat_failed,expected_user_facing",
    [(False, False), (True, True)],
    ids=["default-not-user-facing", "emit_chat_failed-is-user-facing"],
)
def test_exhausted_user_facing_flag_reflects_emit_chat_failed(
    captured_events, emit_chat_failed, expected_user_facing
):
    """APITimeoutError emits exhausted (not retry_scheduled); user_facing tracks the flag."""
    ...
    assert exhausted[0]["properties"]["user_facing"] is expected_user_facing


@pytest.mark.parametrize(
    "make_error,expected_exc,failure_kind,error_type,reason_options",
    [
        (
            ...
            "transient",
            "InternalServerError",
            {"max_attempts", "budget_exhausted", "delay_over_remaining"},
        ),
        (
    ...
    ids=["exhausted-chat-completion", "immediate-timeout"],
)
def test_chat_failed_emitted_on_exhaustion_when_flagged(
    captured_events, make_error, expected_exc, failure_kind, error_type, reason_options
):
    """chat.failed fires when emit_chat_failed=True, for both retry-exhaustion and immediate timeout."""
    with pytest.raises(expected_exc):
        call_openai_with_retry(
            "chat_generate",
            lambda: (_ for _ in ()).throw(make_error()),
            tenant_id="tnt_test",
            bot_id="bot_xyz",
    ...
    props = e["properties"]
    assert props["operation"] == "chat_generate"
    assert props["failure_kind"] == failure_kind
    assert props["error_type"] == error_type
    assert props["call_type"] == "chat_completion"
    assert props["reason"] in reason_options
