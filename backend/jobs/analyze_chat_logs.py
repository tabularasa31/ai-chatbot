...
- Answer selection: the assistant message that answered *this* user message —
  the next assistant row with no other user row in between
...
def _load_messages(
    ...
    Rows are stored with their original wording; this job embeds them and feeds
    them to FAQ/alias generation, all of which are OpenAI calls, so the text is
    redacted here — at the point it enters the job.
    """
...
def _get_answer_for_message(
    ...
    """Return the redacted text of this message's own answer.

    The answer text is redacted: it ends up in FAQ candidates generated by an
    OpenAI call.

    "This message's own answer" means the first assistant message after it
    **with no other user message in between**. Taking simply the next
    assistant row was sound while every user row was immediately followed by
    an assistant one, but a live operator handoff breaks that: ``N``
    consecutive visitor turns are swallowed by ``OperatorHandler`` and get no
    assistant row at all. Unbounded, every one of them would pair with the
    first bot answer after the release — an unrelated one — and all ``N``
    would be fed to OpenAI as FAQ candidates carrying that single answer.
    A visitor turn nobody answered has no answer, and returning ``None``
    drops it from the candidate set, which is correct.
    """
    ...
    next_user_at = (
        db.query(func.min(Message.created_at))
        .filter(
    ...
    answer_q = db.query(Message).filter(
        Message.chat_id == conversation_id,
        Message.role == MessageRole.assistant,
        Message.created_at > user_msg.created_at,
    )
    if next_user_at is not None:
        answer_q = answer_q.filter(Message.created_at < next_user_at)
    assistant_msg = answer_q.order_by(Message.created_at.asc()).first()
    if assistant_msg is None:
        return None

    return redact_for_egress(assistant_msg.content)
...
async def run_job(
    ...
    try:
        ...
        for cluster in clusters:
            ...
            for msg in cluster:
                answer_text = _get_answer_for_message(db, msg.id, msg.conversation_id)
                if answer_text:
                    members.append(ClusterMember(message=msg, answer=answer_text))
