...
For the automatic triggers the bot first asks in one sentence whether the user wants the request forwarded — no ticket exists until they agree, and if they decline the chat simply carries on. Once a ticket is created (immediately, when the user asked for a human outright), a badge with the ticket number appears above the transcript and the bot confirms the handoff; if no email is on file it asks for one, since support replies by email. The chat stays open throughout — the user can keep asking the bot questions while support works the ticket. Only when the user later confirms they need nothing further does the chat close, at which point the widget shows a **"Start new chat"** button with the existing transcript still visible above it.
