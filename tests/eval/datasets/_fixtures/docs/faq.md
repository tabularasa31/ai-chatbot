---
title: FAQ
description: Common questions about Chat9 — setup, widget, quality, identified sessions, and security.
---
...
Chat answers use `gpt-5-mini`; lightweight guard and validation checks use `gpt-4o-mini` by default.
...
PDF, Markdown (`.md`, `.mdx`), Swagger/OpenAPI (`.json`, `.yaml`, `.yml`), Word (`.docx`, `.doc`), and plain text (`.txt`).
...
Yes. If the retrieval confidence is too low to give a reliable answer, Chat9 may ask one short clarification question instead of guessing. The bot asks at most one clarifying question per conversation — after that it either gives a best-effort answer with a caveat or escalates to your support team.
...
Escalation happens automatically or on request:

- **Automatically** — when the bot can't find a relevant answer in your documentation (low retrieval confidence), or when no documents are indexed at all, or when the generated answer fails quality validation.
...
The chat stays open — escalation does not close it. The user can keep asking the bot questions while support works the ticket. After the handoff the bot asks whether it can help with anything else; if the user says no, the bot says goodbye and the chat simply stays open, ready for the next question. A conversation never closes — it only rolls over into a fresh one after a week of silence.
...
**Can I set the support email?**
Yes. Go to **Dashboard → Settings** and enter the email address that should receive escalation notifications.

**Can ticket priority be elevated for premium users?**
Yes, if you use identified sessions. Pass `plan_tier: "enterprise"` or `plan_tier: "pro"` in the identity token. User-request escalations from those users are marked **Critical** (vs High for standard users). See [Embedding the Widget → Identified sessions](/docs/embedding-the-widget#identified-sessions-optional).
