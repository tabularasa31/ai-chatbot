...
from backend.escalation.service import mark_ticket_in_progress, resolve_chat_tickets
...
def resolve_from_operator(
    ...
    *,
    ...
    others it is channel-agnostic: the console button calls it today, the
    e-mail link and the messenger inline button will call it with their
    channels. Two things happen in one transaction:

    * every ticket still being worked in the chat goes ``resolved`` and its
      e-mail reply token is revoked — a closed request is not somewhere an old
      notification should still be able to write;
    * the chat is handed back to the bot, closing the operator's stretch, so
    ...
    tickets = resolve_chat_tickets(db, chat_id=chat.id, resolution_text=resolution_text)
    stretch = None
