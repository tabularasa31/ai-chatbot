...
A ticket badge appears above the transcript, and the bot confirms the handoff. The chat stays open — the visitor can keep asking questions while support works the ticket. After the handoff the bot asks whether anything else is needed; only when the visitor says no does the chat close, at which point the widget shows a **"Start new chat"** button with the existing transcript still visible above it.
