...
@pytest.mark.parametrize(
    ("llm_lang_retry_ms", "expected_llm_ms"),
    [
        pytest.param(11_000, 17_000, id="lang_retry_ms_summed_into_llm_ms"),
        pytest.param(0, 5_000, id="no_retry_reports_llm_ms_unchanged"),
    ],
)
def test_chat_turn_llm_ms_includes_lang_retry(monkeypatch, llm_lang_retry_ms, expected_llm_ms):
    ...
    _emit_chat_turn_event(
        ...
        latency_ms=20_000,
        retrieval_ms=2_000,
        llm_ms=5_000 if llm_lang_retry_ms == 0 else 6_000,
        llm_lang_retry_ms=llm_lang_retry_ms,
    )

    turn = next(c for c in client.calls if c["event"] == "chat.turn")
    assert turn["properties"]["llm_ms"] == expected_llm_ms
    assert turn["properties"]["llm_lang_retry_ms"] == llm_lang_retry_ms
...
@pytest.mark.parametrize(
    ("strategy", "reject_reason", "is_reject", "reliability_score", "expected_cap_reason"),
    [
        pytest.param("guard_reject", "not_relevant", True, None, "not_relevant", id="from_reject_reason"),
        pytest.param("rag_only", None, False, "low", "low_confidence", id="from_low_reliability"),
        pytest.param("rag_only", None, False, "high", None, id="none_when_high"),
    ],
)
def test_chat_turn_cap_reason_derivation(
    monkeypatch, strategy, reject_reason, is_reject, reliability_score, expected_cap_reason
):
    svc, client = _make_enabled_svc(monkeypatch)
    ...
    completed = next(c for c in client.calls if c["event"] == "chat_completed")
    assert completed["properties"]["cap_reason"] == expected_cap_reason
