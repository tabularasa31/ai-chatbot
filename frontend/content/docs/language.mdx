...
Languages cannot be switched mid-conversation. A new language is resolved only in a new conversation: after the idle timeout, after the chat was closed following an escalation and the end user clicked **Start new chat**, or when your integration starts a new session. The new session goes through the full chain again and may settle on a different language. This is intentional: real bilingual switches mid-conversation are rare in support, and the lock prevents the much-more-common annoyance of the bot flip-flopping between languages on noisy turns.
