...
- **`guard_events` table** (`models/guard.py`, recorder in `guards/events.py`): every primary-path verdict (injection + relevance) is written as one row — kind, blocked, reason, score, `evidence_hash` (SHA-256 of the trigger, never raw text), latency, cache_hit, chat_id — plus a `guard.verdict` PostHog event for the "Guards FP/FN" dashboard. The write is fire-and-forget on its own `AsyncSession` and best-effort (never breaks chat). `label` (fp/fn) is reserved for a future manual-review tool.
