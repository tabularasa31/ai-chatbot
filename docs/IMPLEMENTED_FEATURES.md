...
| Knowledge API (Profile + FAQ) | Tenant knowledge endpoints for profile and FAQ moderation: `GET/PATCH /knowledge/profile`, `GET /knowledge/faq`, `POST /knowledge/faq/{id}/approve`, `POST /knowledge/faq/{id}/reject`, `POST /knowledge/faq/approve-all`, `PUT/DELETE /knowledge/faq/{id}`; extraction status exposed as profile field; public profile terminology now uses `topics` for extracted doc themes while retaining `modules` as a compatibility alias | `backend/knowledge/routes.py`, `backend/knowledge/schemas.py`, `backend/main.py`, `backend/models.py`, `tests/test_knowledge_api.py` |
...
| Knowledge subtabs (Phase 3 UI) | `/knowledge` now supports tabbed views: `Documents` (existing), `Profile` (`?tab=profile`) with extracted profile editing, extracted topics editing, extraction status, and glossary accordion, and `FAQ` (`?tab=faq`) with moderation workflow (accept/reject/accept-all/edit, pending counter, filters, extraction-driven polling) | `frontend/app/(app)/knowledge/page.tsx`, `frontend/lib/api.ts` |
