...
def build_rag_prompt(
    ...
    *,
    ...
    system_rules = (
        ...
        "- The user's own checks come before the handoff. When your reply gives the user steps they can carry out themselves — a setting to change, an option to switch, something to verify on their side — and they have not said they already did them, end the reply with those steps as a short numbered checklist, ask them to run it and tell you what happened, and append the literal marker `<checklist/>` as the very last token. This holds even when part of the request needs a person or is not covered by the documentation: say which part in the same reply — it reaches support together with the user's results. Such a reply carries no `<needs_human/>`, `<offered_ticket/>` or `<clarifying/>`: the checklist is its only question. Give the checklist at most once per conversation, and skip it when there is nothing the user can do themselves; once the user reports the results or says they cannot run the steps, the handoff rules above apply as usual.\n"
    )
