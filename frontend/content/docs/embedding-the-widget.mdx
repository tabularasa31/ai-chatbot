...
`/widget/chat` responds with `Content-Type: text/event-stream` (Server-Sent Events), not a single JSON body. The server emits one or more `data: {"type":"chunk","text":"…"}` frames as the model streams, then exactly one terminal `data: {"type":"done", "text":"<full reply>", "session_id":"…", "chat_ended":false}` frame. The `done` payload may also include `ticket_number` (when the turn opened a support ticket) and `sources` (when the answer cited URL-backed documents). On a server-side error you may instead receive a single `{"type":"error","code":…,"message":"…"}` frame before the stream closes. See [API → POST /widget/chat](/docs/api#post-widgetchat) for the full contract.
