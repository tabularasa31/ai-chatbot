...
1. **`userHints.locale`** — if you set `userHints` on `window.Chat9Config` and included a `locale`, that wins. It's the strongest signal because your own system told us about this specific visitor.
...
If your tenant frontend already knows the visitor's preferred language (for example, a registered locale on their account profile), pass it via `userHints.locale` on `window.Chat9Config`. Chat9 will use it as the bootstrap greeting language and as a stronger initial signal than the browser default. See [Embedding the Widget → userHints](/docs/embedding-the-widget#userhints-optional).
