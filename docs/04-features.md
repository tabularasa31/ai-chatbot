...
Four owner-only routes manage the team: `POST /tenants/members/invite`, `GET /tenants/members`, `PATCH /tenants/members/{id}`, `DELETE /tenants/members/{id}`. The last owner can be neither removed nor demoted, and nobody can remove themselves.
