...
| K8 | После `chat_ended=true` widget показывает `Start new chat` | input больше не застревает в тупике; действие видно; старый transcript остаётся на экране |
| K9 | Нажать `Start new chat`, затем отправить новый вопрос | создаётся новая сессия, старый закрытый чат не продолжается, новый разговор начинается ниже после separator |
...
- `Start new chat` не должен стирать уже показанную историю до reload
