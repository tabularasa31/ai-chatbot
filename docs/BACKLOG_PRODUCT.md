...
### [FI-KYC] Know Your Customer — User Identification ✅ DONE (2026-03-21)
**Why:** Switching Cost moat; без идентификации нельзя полноценно строить эскалации, disclosure по аудитории и персонализацию.

**Реализация (v1):**
- Тенант на **своём бэкенде** подписывает payload (в т.ч. `user_id`, опционально email/name/plan/…) HMAC-SHA256 секретом; в payload обязательны `tenant_id` (= `public_id`), `exp`/`iat`.
- Виджет/интеграция вызывает `POST /widget/session/init` с `api_key` и опционально `identity_token` → `session_id` + `mode` (`identified` | `anonymous`). Невалидный токен **не ломает** виджет (anonymous).
- Контекст хранится в `chats.user_context` (JSON). В системный промпт RAG попадают **только** `plan_tier`, `locale`, `audience_tag` (не email/user_id/name).
- Дашборд: **Settings → Widget API** (`/settings/widget`) — генерация/ротация секрета, метрики identified-сессий за 7 дней; API `GET/POST /clients/me/kyc/*`.
- **v2 continuity shipped:** identified users resume on `POST /widget/session/init` by `client_id + user_id` (24h, only open chats), anonymous users keep browser-local continuity, closed chats show `Start new chat`.
- `user_sessions` now stores the active identified-user lifecycle and turn counts across widget/chat flows.
- **Тесты:** `tests/test_kyc.py`. **Миграция:** `fi_kyc_user_identification`.
...
### ~~[FI-ESC] L2 Escalation Tickets~~ ✅ Done (2026-03-21)
**Shipped (v1):** внутренние тикеты в БД (`escalation_tickets`), дашборд **`/escalations`**, email владельцу тенанта (Brevo), ручной эскейл **`POST /chat/{session_id}/escalate`** (X-API-Key) и **`POST /widget/escalate`** (public `client_id` + `session_id`). Триггеры: низкая схожесть / нет чанков / явный запрос человека / ручной эскейл (в т.ч. «не помогло» через API). UX эскалации — через отдельный OpenAI JSON-turn (`backend/escalation/openai_escalation.py`); в ответе может быть строка `[[escalation_ticket:ESC-####]]`. Состояние чата: `escalation_awaiting_ticket_id`, `escalation_followup_pending`, `ended_at`; в API чата — поле **`chat_ended`**. Локаль виджета: `locale` в `POST /widget/session/init`, query `locale` на `/widget/chat`, заголовок **`X-Browser-Locale`** на `POST /chat`. Миграция: `fi_esc_v1`. Тесты: `tests/test_escalation.py`, обновлённый `conftest` (мок эскалационного OpenAI). Чеклист QA: **`docs/qa/FI-ESC-escalation-tickets-qa.md`**. Спека-промпт: `cursor_prompts/FI-ESC-escalation-tickets.md` (можно хранить как архив).

**v2 (backlog):** Zendesk/Intercom integration.

---

### ~~[FI-DISC] Disclosure Controls~~ ✅ Phase 1 done (2026-03-21)
**Shipped (v1):** один уровень детализации ответа на тенант (`detailed` | `standard` | `corporate`), hard limits в промпте, `clients.disclosure_config`, `GET`/`PUT /clients/me/disclosure`, UI **Response controls**. Чтение: `level` или алиас `default_level`; запись: только `level`. Код: `backend/disclosure_config.py`, `backend/chat/service.py`, миграция `fi_disc_v1`. Тесты: `tests/test_disclosure.py`. Промпт после merge удалён; описание: `PROGRESS.md`, `docs/IMPLEMENTED_FEATURES.md`.

**Backlog (будущие фазы):** preview/simulation endpoint.
...
Реализация: `backend/chat/pii.py`, `backend/chat/service.py` (`process_chat_message`, `run_debug`), `tests/chat/test_pii.py`. Подробности и Stage 2: `docs/BACKLOG_SECURITY.md` (FI-044). Промпт `cursor_prompts/FI-043-pii-redaction-regex.md` удалён после внедрения.
