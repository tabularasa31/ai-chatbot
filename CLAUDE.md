...
- **Backend**: Railway — `alembic upgrade head` runs automatically on each deploy, in the service's start/pre-deploy command (the Procfile `release` line is not honoured by Railway). See **AGENTS.md → "Deployment safety"** for the migration-head rule, the `main` ruleset and the Sentry heartbeat.
