...
- **Public widget endpoints** (`/widget/*`, `/widget/config`) —
  unauthenticated, identified by your bot's `public_id`. This is what
  the embedded chat widget on your site calls. The loader script
  itself is served from `widget.getchat9.live/widget.js`, separate
  from the API host. Documented in detail below.
...
HTTP endpoints. You don't normally call them yourself — the loader
script handles everything — but they're documented here for custom
...
server emits three kinds of events while it works:

- `data: {"type":"status","stage":"..."}` — progress signal (e.g.
  `"thinking"`), emitted before and during answer generation. Safe to
  ignore if you only want the answer text.
...
- `data: {"type":"done","text":"<full reply>","session_id":"...","chat_ended":false}` —
  ...
  escalation). The final payload can also include `ticket_number` when
  the turn opened a support ticket and `sources` when the answer cited
  URL-backed documents.

If something goes wrong server-side you may instead get one
`data: {"type":"error","code":...,"message":"..."}` event before the
stream closes.
...
**Response:** JSON object containing the `session_id`, ordered
`messages` (`role` + `content`), `chat_ended`, and optional
`ticket_number`.
...
```json
{
  "user_note": "Optional free-form text from the user",
...
The loader is **not** served from the API host. It lives at
`https://widget.getchat9.live/widget.js` (a separate Vercel
project). Embed it on your site with:
...
You don't normally fetch this directly — the script tag does it. See
[Embedding the Widget](/docs/embedding-the-widget) for the full
attribute reference.
...
| Method | Path | What it does |
|--------|------|--------------|
| POST | `/chat` | Server-side chat using the tenant's `ck_...` widget API key — same answer pipeline as the widget, returned as JSON with `text`, `session_id`, `chat_ended`, optional `ticket_number`, and trace fields `source_documents` / `tokens_used`. |
