...
3. Supported file formats: PDF, Markdown (`.md`, `.mdx`), HTML (`.html`, `.htm`), Swagger/OpenAPI (`.json`, `.yaml`, `.yml`), Word (`.docx`, `.doc`), plain text (`.txt`). Maximum file size: 50 MB.
...
The first script registers the widget; the second mounts it. See [Embedding the Widget](/docs/embedding-the-widget) for configuration options and the full `Chat9Widget` API (`start`, `stop`, `setHints`, `destroy`).
