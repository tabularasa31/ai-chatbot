...
Escalating to a human agent does not end the conversation — the visitor can keep writing, and the bot keeps answering, while the ticket is open. The chat is marked as ended only when the visitor confirms they need nothing further (the bot asks "anything else I can help with?" after the handoff and the visitor says no). At that point the transcript stays visible; a "Start new chat" button appears for the visitor to open a new session.
