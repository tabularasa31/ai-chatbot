...
**What a seat gates today:** `require_seated_member` on the three `/operator/*` routes — take a chat, answer in it, release it. A seatless caller gets **403** whatever their role. `backend/seats/` also carries two read helpers the inbound e-mail lane decides on: `tenant_has_any_seat(tenant_id=…)` picks the notification's `Reply-To` (our token address when the workspace holds a seat, the visitor's own address when it holds none), and `user_holds_seat(user_id=…)` decides what happens to a mailed reply — a seat holder's answer enters the conversation, anyone else's is forwarded to the visitor by mail and leaves only a stamp on the ticket (`forwarded_reply_at` / `forwarded_reply_from`, shown in the inbox as "Answered by e-mail" and counted as an answer by the queue — the conversation leaves "Waiting" until the visitor asks for a human again), never a message. The forward never enters `first_response_ms`; its own clock is `response_ms` on the `email_lane.reply_forwarded` event. Full rules in `docs/docs-ru/09-operator-handoff.md` § Почтовая полоса.
...
Every bot-detected retrieval verdict gives the user a second chance before
...
- `low_similarity` and the `no_documents` **slow path** share one tracker
  ...
  between them still reaches the handoff on its second weak turn.
...
the session ended (`chats.session_ended_event_at`), so a user resuming days
...
A reply forwarded to the visitor by mail from an address holding no seat opens
no stretch and is never part of `first_response_ms` or `answered` — the product
metric measures the product. Its own clock is `response_ms` on the
`email_lane.reply_forwarded` event: from the request being raised (`created_at`,
or `requested_again_at` after a repeat ask) to the forward. Analytics may add the
two together; the event never does.
...
`no_documents` is deferred on both paths — see "Two-strike deferral" above: the fast path asks the user to rephrase once before escalating; the slow path (`escalation/service.py:190-191` via `steps/generate.py:700`) delivers the generated answer on the first zero-chunk turn and shares the `low_similarity` tracker, so the second weak turn of either flavour escalates.
...
3. The widget calls `POST /widget/session/init` with `{ bot_id, user_hints }`. In bubble mode this happens on the first open, not on page load — the loader does not load the widget iframe until the visitor clicks the bubble (inline mode loads immediately, since the widget is visible).
