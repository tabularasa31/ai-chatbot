...
PDF, Markdown (`.md`, `.mdx`), HTML (`.html`, `.htm`), Swagger/OpenAPI (`.json`, `.yaml`, `.yml`), Word (`.docx`, `.doc`), and plain text (`.txt`).
...
- Read real conversations in the Inbox to spot answers that missed.
...
For the automatic triggers the bot first asks in one sentence whether the user wants the request forwarded — no ticket exists until they agree, and if they decline the chat simply carries on. Once a ticket is created (immediately, when the user asked for a human outright), a badge with the ticket number appears above the transcript and the bot confirms the handoff; if no email is on file it asks for one, since support replies by email. The chat stays open throughout — the user can keep asking the bot questions while support works the ticket. After the handoff the bot asks whether it can help with anything else; if the user says no, the bot says goodbye and the chat simply stays open, ready for the next question. A conversation never closes — it only rolls over into a fresh one after a week of silence.

**What does the operator see?**
The conversation appears under **Dashboard → Inbox → Needs attention** with the trigger type and a sequential ticket number (e.g. `ESC-42`). From there an operator with a seat reads the full conversation, replies to the visitor directly in the widget, and marks the ticket resolved. A notification email is also sent to the support email configured in Settings, with a link to the conversation.
...
By default the chat widget is anonymous. An identified session lets your server attach a verified user identity (user id, email, plan, etc.) to a conversation. This information appears in the Inbox and can be used for routing and personalisation.
