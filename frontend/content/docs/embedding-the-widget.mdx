...
| `apiBase` | inferred from script origin | Chat9 API origin. You normally do **not** set this — it's inferred from the loader's own origin (`https://getchat9.live` for the default loader, `https://api-ru.getchat9.live` when the loader is served from the [RU edge](#routing-from-russia)). Override only for staging/dev. |
| `widgetBase` | `<script origin>/v1/` | Origin of the widget UI bundle. Defaults to the loader script's own origin. Used internally for custom widget builds. |
