...
| Method | Path | What it does |
|--------|------|--------------|
| POST | `/chat` | Server-side chat using the tenant's `ck_...` widget API key — same answer pipeline as the widget, returned as JSON with `text`, `session_id`, `chat_ended`, optional `ticket_number`, and private trace fields `source_documents` / `tokens_used`. |
