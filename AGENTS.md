...
| Auth | JWT, bcrypt, email verification (Brevo HTTP API); successful `/auth/verify-email` provisions the user's single client/workspace; internal eval QA (`/eval/*`) uses a separate signing secret `EVAL_JWT_SECRET` |
