...
@pytest.mark.parametrize(
    "case_id",
    ["new_ticket_notify", "advance_notification_marker", "ticket_update_notify"],
)
def test_naive_last_notified_at_contract(
    tenant: TestClient,
    db_session: Session,
    case_id: str,
) -> None:
    ...
    ZONE`` in Postgres. The notify helpers used to write ``datetime.now(UTC)``
    ...
    :func:`backend.models.base._utcnow`. This asserts the contract on all
    three call sites that assign ``ticket.last_notified_at``: the initial
    new-ticket notify, the email-capture marker fixup, and the threaded
    follow-up update notify.
    """
    if case_id == "new_ticket_notify":
        cl = _make_tenant_for_email_test(
        ...
        with patch(
            ...
            _notify_tenant_new_ticket(cl, ticket, db_session)
    elif case_id == "advance_notification_marker":
        cl = _make_tenant_for_email_test(
        ...
        db_session.add(
            Message(chat_id=chat.id, role=MessageRole.user, content="anchor turn")
        )
        ticket = EscalationTicket(
            tenant_id=cl.id,
