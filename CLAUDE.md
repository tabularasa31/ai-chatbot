...
- **Through-the-app first.** Default to a test that drives a real request through the FastAPI app (the style of `test_chat_api.py`, `test_chat_escalation.py`, `test_search.py`) over one that mocks internals to isolate a unit. A new feature or behaviour change lands with a scenario in the domain's existing test file that proves it works through the app and can be rerun to catch regressions — that scenario is the artifact.
...
- **Landing in a `make` suite.** `smoke`, `escalation` and `rag-edge` are assembled by file list plus a `-k` substring filter in the Makefile (e.g. `escalat`, `openai_unavailable`, `low_vector`). A scenario belongs to a suite only if its file is listed there and its name matches the filter — check the target before naming the test, and extend the filter when a new failure class appears. Every test must be reachable through one of the `make` targets in this file.
