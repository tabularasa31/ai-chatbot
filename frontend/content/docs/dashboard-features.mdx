...
- View a log of all PII redaction events — every time a message contained detected personal information (email, phone, etc.) that was masked before leaving the platform: on requests to OpenAI, on escalation tickets, and on notification emails.
- Download the log as a CSV for audit or compliance purposes.
- Choose which optional entity types (identity documents, IP addresses, tokenised URLs) are masked at those boundaries. Email addresses, phone numbers, API keys, passwords, and payment cards are always masked.
- Your conversations are stored as they were written, so chat logs and escalations in the dashboard show the original wording — masking applies to what leaves the platform, not to what you read back.
...
**One ticket per conversation:** if the same visitor asks for a human again in the same conversation, the new message is added to the existing ticket and threaded under the original notification email — you won't get a new ticket number for every repeat.

**Resolving tickets:** Click **Resolve** on any open ticket to close it. Resolved tickets remain visible with a filter toggle.

**Auto-closed tickets:** when a conversation goes quiet for long enough, its ticket is moved to **Auto-closed** so the open queue stays meaningful. This only clears the queue — it does not mean anyone replied. Auto-closed tickets stay visible under the **Auto-closed** filter, and you can still mark one **Resolved** once your team has handled it.
